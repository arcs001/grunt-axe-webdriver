...
module.exports = function (grunt, WebDriver, Promise, AxeBuilder, reporter) {
	...
	if (this.data.loginurl) {
		...
		driver.get(this.data.loginurl)
			.then(function () {
				...
				driver.wait(webdriver.until.titleContains(this.data.titleToWait), 1000, "Failed to wait for login redirect");
	...
	Promise.all(this.data.urls.map(function (urlAndScript) {
		return new Promise(function (resolve, reject) {
			var config = urlAndScript.split(';');
			var url = config[0];
			var preScriptPath = config[1];
